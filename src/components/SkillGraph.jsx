...
// Click a node to highlight it. Drag nodes around freely.
...
export default function SkillGraph({ skills = [], experience = [] }) {
  ...
  useEffect(() => {
    ...
    const nodes = skills.map((skill, i) => ({
      ...
      // Give "center" node more gravity to keep graph together
      size: Math.random() * 6 + 14, // random size for visual variety
    }));

    // Create links: connect skills that appear together in experience bullets
    const links = [];
